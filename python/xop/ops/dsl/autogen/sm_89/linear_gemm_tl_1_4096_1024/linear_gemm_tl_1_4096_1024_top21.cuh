...
template <typename T,
  ...
  for (int k = 0; k < 8; ++k) {
    ...
    for (int i_2 = 0; i_2 < 8; ++i_2) {
      *(uint4*)(((bfloat16_t*)buf_dyn_shmem) + ((((((((((int)threadIdx.x) & 15) >> 3) * 4096) + (i_2 * 512)) + ((((int)threadIdx.x) >> 4) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(B + (((((((int)bx) * 65536) + (i_2 * 8192)) + ((((int)threadIdx.x) >> 4) * 1024)) + (k * 128)) + ((((int)threadIdx.x) & 15) * 8)));
    }
    __syncthreads();
  ...
  for (int i_3 = 0; i_3 < 4; ++i_3) {
    ...
    *(uint1*)(((bfloat16_t*)buf_dyn_shmem) + (((((((i_3 & 1) * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((((int)threadIdx.x) >> 5) * 16) + ((i_3 >> 1) * 8)) >> 5) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 15) >> 3)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + (i_3 >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(uint1*)(C_shared_local_cast + 0);
  }
  __syncthreads();
...
// selected_hparams: [16, 64, 128, 1, 0, 128, <GemmWarpPolicy.Square: 0>, False].
// smem: 20480 bytes.
...
#define LAUNCH_INFO_linear_gemm_tl_1_4096_1024 dim3(64, 1, 1), dim3(128, 1, 1), 20480, stream

// latency: 0.04796 ms vs [ref-0.04762 sim-1.0], idx: 33