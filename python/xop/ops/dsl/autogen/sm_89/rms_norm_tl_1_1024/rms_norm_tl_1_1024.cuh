...
__device__ __forceinline__ void rms_norm_kernel_1_1024(const int bx, const int by, const int bz,
  ...
  *(uint4*)(((bfloat16_t*)buf_dyn_shmem) + ((((int)threadIdx.x) * 8) + 1024)) = *(uint4*)(B + (((int)threadIdx.x) * 8));
  ...
  #pragma unroll
  for (int i = 0; i < 2; ++i) {
    *(uint2*)(A_shared_local_cast + 0) = *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + ((i * 512) + (((int)threadIdx.x) * 4)));
    float4 __1;
  ...
  for (int i_1 = 0; i_1 < 2; ++i_1) {
    *(uint2*)(B_shared_local_cast_1 + 0) = *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + (((i_1 * 512) + (((int)threadIdx.x) * 4)) + 1024));
    float4 __2;
  ...
  for (int i_4 = 0; i_4 < 2; ++i_4) {
    ...
    *(uint2*)(C + ((i_4 * 512) + (((int)threadIdx.x) * 4))) = *(uint2*)(C_local_cast_2 + 0);
  }
}
...
extern "C" int create_rms_norm_tl_1_1024(bfloat16_t* __restrict__ A, bfloat16_t* __restrict__ B, bfloat16_t* __restrict__ C) {

	return 0;
}
#define LAUNCH_INFO_rms_norm_tl_1_1024 dim3(1, 1, 1), dim3(128, 1, 1), 4096, stream

// latency: 0.00977 ms vs [ref-0.05023 sim-0.99999], idx: 0