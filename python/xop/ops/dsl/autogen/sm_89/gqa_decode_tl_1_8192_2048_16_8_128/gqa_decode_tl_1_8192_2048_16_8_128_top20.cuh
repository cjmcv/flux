  ...
  if ((((int)threadIdx.x) % 4) == 0) {
    ...
    for (int i_32 = 0; i_32 < 2; ++i_32) {
      if (((((((int)threadIdx.x) >> 5) * 8) + (i_32 * 4)) + ((((int)threadIdx.x) & 31) >> 3)) < 1) {
        glse[((((((((int)threadIdx.x) >> 5) * 128) + (i_32 * 64)) + (((int)by) * 16)) + (((((int)threadIdx.x) & 31) >> 2) * 8)) + ((int)bz))] = ((bfloat16_t)logsum[i_32]);
      }
    }
  }
  __syncthreads();
...
// selected_hparams: [128, 64, 8, 1, 128].
// smem: 0 bytes.
// use_cooperative_groups: 0.
// layout: (1, 8, 8), (128, 64, 8), (16, 1, 1), (128, 64, 8)
// block_dim=(128, 1, 1).
...
// latency: 0.07286 ms vs [ref-0.05556 sim-0.99973], idx: 36