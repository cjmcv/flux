  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
  ...
  if (0 < valid_kv_seqlen) {
    ...
    for (int i_3 = 0; i_3 < 4; ++i_3) {
      tl::cp_async_gs<16>((&(((bfloat16_t*)buf_dyn_shmem)[(((((((((((int)threadIdx.x) & 15) >> 3) * 2048) + (i_3 * 512)) + ((((int)threadIdx.x) >> 4) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) + 12288)])), (&(V[((((i_3 * 8192) + ((((int)threadIdx.x) >> 4) * 1024)) + (((int)by) * 128)) + ((((int)threadIdx.x) & 15) * 8))])));
    }
    tl::cp_async_commit();
  }
  for (int k = 0; k < (((valid_kv_seqlen + 31) >> 5) - 1); ++k) {
    #pragma unroll
  ...
  if (1 <= valid_kv_seqlen) {
    ...
    for (int i_20 = 0; i_20 < 16; ++i_20) {
      float condval_2;
      if ((((((((valid_kv_seqlen + 31) >> 5) * 32) + ((i_20 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_20 & 1)) < (valid_kv_seqlen + 32))) {
...
// selected_hparams: [32, 64, 1, 1, 128].
// smem: 32768 bytes.
...
#define LAUNCH_INFO_gqa_decode_tl_1_8192_2048_16_8_128 dim3(1, 8, 1), dim3(128, 1, 1), 32768, stream

// latency: 0.10284 ms vs [ref-0.05535 sim-0.99968], idx: 3