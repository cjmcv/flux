...
// struct LayoutDetailsB<TypeA, cutlass::uint4b_t, Arch>
// {
//     static constexpr int ThreadblockK = 128 * 8 / cutlass::sizeof_bits<TypeA>::value;

// private:
//     static constexpr int ElementsPerCacheLine = 128 * 8 / cutlass::sizeof_bits<cutlass::uint4b_t>::value;
//     static constexpr int ColumnsInterleaved = ElementsPerCacheLine / ThreadblockK;

// public:
//     using Layout = cutlass::layout::ColumnMajorTileInterleave<ThreadblockK, ColumnsInterleaved>;
//     static constexpr int ElementsPerAccess = 128 / cutlass::sizeof_bits<cutlass::uint4b_t>::value;
...
//     details.uses_imma_ldsm = false;//std::is_same<MmaOperator, cutlass::arch::OpMultiplyAddDequantizeInterleavedBToA>::value;
//     return details;
// }
...
//     else
//     {
//         assert("Invalid quantization type for LDSM permutation");
//     }
// }
...
//     assert(shape.size() == 2 || shape.size() == 3);//, "Shape must be 2-D or 3-D");
//     const size_t num_experts = shape.size() == 2 ? 1 : shape[0];
//     const size_t num_rows = shape.size() == 2 ? shape[0] : shape[1];
//     const size_t num_cols = shape.size() == 2 ? shape[1] : shape[2];
...
//     // }
//     // else 
//     if (quant_type == QuantType::W4_A16 || quant_type == QuantType::W4_AFP8)
//     {
...
//     else
//     {
//         printf("Invalid quantization type for interleaving.");
//     }
// }
    ...
    bool force_interleave)
{
    ...
    assert(processed_quantized_weight); // , "Processed quantized tensor is NULL"
    assert(scale_ptr);//, "Scale output pointer is NULL");
    assert(input_weight_ptr);//, "Input weight pointer is NULL");
    assert(shape.size() == 2 || shape.size() == 3);//, "Shape must be 2-D or 3-D");
