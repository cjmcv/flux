...
public:
  ...
  Status run(cudaStream_t stream = nullptr) {
    int splitk_chunk = 2;  // 2 [hardcode] 
    dim3 block = dim3(kThreadsPerRow, kThreadCount / kThreadsPerRow, 1); // (16, 8£¬ 1)
    dim3 grid = dim3((params_.problem_size.row() + (block.y - 1)) / block.y, 1, splitk_chunk); // (m//8, 1£¬ 1)
...
public:
  ...
  void operator()(Params const &params, SharedStorage &shared_storage) {
    ...
    for (int batch_idx = blockIdx.z; batch_idx < splitk_chunk; batch_idx += gridDim.z) {
      ...
      if (idx_row_m < params.problem_size.row()) {
        ...
        ElementA const *ptr_A = params.ref_A.data() + batch_idx * params.problem_size.column() / splitk_chunk;   // 2 [hardcode]
        ElementB const *ptr_B = params.ptr_B + batch_idx * params.problem_size.column() / splitk_chunk;
        ...
        int unroll_col_k = batch_idx * tile_align_col / splitk_chunk;

        // for (; unroll_col_k < params.problem_size.column() / tileA_k * tileA_k; unroll_col_k += tileA_k) {
        for (; unroll_col_k < (batch_idx+1) * tile_align_col / splitk_chunk; unroll_col_k += tileA_k) {
        ...
        for (int k = unroll_col_k + idx_col_k; k < (batch_idx+1) * params.problem_size.column() / splitk_chunk; k += kThreadsPerRow) {
...
class GemvSimtImpl : public GemmBase {
  ...
  // using EpilogueOpIdentity = cutlass::epilogue::thread::LinearCombination<
  //   ElementC,
  //   1,
  //   ElementAccumulator,
  //   ElementAccumulator>;
