  ...
  if constexpr (FUSE_RES == true) {
    if constexpr (M == 1) {
      if constexpr (N == 1024 && K == 3072) {
        linear_gemm_add_tl_1_1024_3072<T, THREAD_NUM, TILE_DIM_X, TILE_DIM_Y, TILE_DIM_Z, M, N, K>(
          bx, by, bz, input_ptr, weight_ptr, residual_ptr, output_ptr); return;
      }
      else if constexpr (N == 1024 && K == 2048) {
        linear_gemm_add_tl_1_1024_2048<T, THREAD_NUM, TILE_DIM_X, TILE_DIM_Y, TILE_DIM_Z, M, N, K>(
          bx, by, bz, input_ptr, weight_ptr, residual_ptr, output_ptr); return;
      }
    } 
  }
    ...
    if constexpr (M == 1) {
      if constexpr (N == 6144 && K == 1024) {
        linear_gemm_tl_1_6144_1024<T, THREAD_NUM, TILE_DIM_X, TILE_DIM_Y, TILE_DIM_Z, M, N, K>(
          bx, by, bz, input_ptr, weight_ptr, residual_ptr, output_ptr); return;
      }
      else if constexpr (N == 4096 && K == 1024) {
        linear_gemm_tl_1_4096_1024<T, THREAD_NUM, TILE_DIM_X, TILE_DIM_Y, TILE_DIM_Z, M, N, K>(
          bx, by, bz, input_ptr, weight_ptr, residual_ptr, output_ptr); return;
      }
    }
  }
