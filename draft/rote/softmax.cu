...
int softmax(const float* x, int len, float* output) {
    // 输入合法性检查
    if (x == NULL || output == NULL || len <= 0) {
        fprintf(stderr, "Error: Invalid input (x=NULL/output=NULL/len<=0)\n");
        return -1;
    }

    // 步骤1：找到向量最大值（数值稳定核心：避免 exp 溢出）
    float max_val = x[0];
    for (int i = 1; i < len; ++i) {
        if (x[i] > max_val) {
            max_val = x[i];
    ...
    // 步骤2：计算调整后指数（x_i - max_val），并累加指数和
    float exp_sum = 0.0f;
    for (int i = 0; i < len; ++i) {
        output[i] = expf(x[i] - max_val);  // expf 是 float 版 exp，效率更高
        exp_sum += output[i];
    }
