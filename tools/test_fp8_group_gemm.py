...
def construct_grouped_and_flat_fp8(
    ...
    print("inner0", x_fp8_grouped[0].shape)
...
def calculate_diff(m: int, n: int, k: int, num_groups: int):
    ...
    # m_indices维度是[m_org*topk]，每个元素表示该行对应的组号。这里忽略topk，使m=m_org*topk.
    # 如 num_groups为8，m为200，m_per_group是25，首先是生成[0,...,7]，每个元素重复25次，表示A矩阵的0-24行对应第0组，25-29对应第1组...
    m_indices = torch.arange(0, num_groups, device="cuda", dtype=torch.int) 
    m_indices = (
        m_indices.unsqueeze(-1).expand(num_groups, m_per_group).contiguous().view(-1)
    )
    print("m_indices: ", m_indices.shape)
    print("x_fp8_grouped: ", x_fp8_grouped[0].shape, x_fp8_grouped[1].shape)
    print("y_fp8_grouped: ", y_fp8_grouped[0].shape, y_fp8_grouped[1].shape)
    print("out_torch: ", out.shape)
    ...
    print("x_fp8_grouped_list: ", x_fp8_grouped_list[0].shape, x_fp8_grouped_list[1].shape)
    print("y_fp8_grouped_list: ", y_fp8_grouped_list[0].shape, y_fp8_grouped_list[1].shape)
    print("out: ", out.shape)
    ...
    # print(y_fp8_grouped_list)

    ctlop_gemm.grouped_forward(
        x_fp8_grouped_list,
        y_fp8_grouped_list,
        outputs=out_ctlop_list,
        inputs_scale=x_fp8_grouped_scale_list,
        weights_scale=y_fp8_grouped_scale_list,
        tuning = None,
    )
    ...
    deepgemm_torch_diff = torch.abs(out_deepgemm - out_torch).mean().item()
    triton_torch_diff = torch.abs(out_triton - out_torch).mean().item()
    deepgemm_triton_diff = torch.abs(out_deepgemm - out_triton).mean().item()
