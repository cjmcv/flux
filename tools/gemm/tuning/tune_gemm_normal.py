...
class GemmBlockScaleFp8Sm89Schema:
    ...
    def gen_scale(self, input: torch.Tensor, weight: torch.Tensor):
        ...
        return x, x_scale, y, y_scale.t().contiguous()
    def get_ref_output(self, input: torch.Tensor, weight: torch.Tensor, 
                       input_scale: torch.Tensor, weight_scale: torch.Tensor,
                       bias: torch.Tensor):
        # output = torch.matmul(input, weight.t())
...
def get_tuning_space(schema):
    space_G = [1]
    space_M = [1,2,4,8,16,32,64,128,256,512,1024,2048,4096] #1,2,4,8,16,32,64,128,256,512,,1024,2048,4096,8192,16384,32768,65536 [8192] # list(range(1, 31)) # [8,16,32,64,128,512,1024] #, 2048, 4096   # , 16384
    space_NK = [(6144,1024)] # [(2560,9728), (6144,2560), (2560,4096), (19456,2560)] # qwen3_4b
    # space_NK = [(128, 4096), (4096, 4096)] #(576, 7168) (3584,5120), (5120,2560), (5120,13824), (27648,5120), 49152
